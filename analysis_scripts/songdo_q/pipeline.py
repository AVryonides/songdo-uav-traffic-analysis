...
from .space_time import (
    ...
    plot_q_composite_space_time_diagrams,
    write_q_model_validation_tables,
)
...
def main():
    ...
    # ── Stitched thesis composite space-time diagrams ─────────────────
    print("\n=== GENERATING COMPOSITE SPACE-TIME DIAGRAMS ===")
    try:
        plot_q_composite_space_time_diagrams(
            general_dir=general_dir,
            title_prefix=f"{INTERSECTION_ID} Intersection",
        )
    except Exception as e:
        print(f"  [ERROR] Composite space-time diagrams: {e}")

    # ── Thesis mathematical model / signal validation tables ──────────
    print("\n=== GENERATING Q MODEL VALIDATION TABLES ===")
    try:
        write_q_model_validation_tables(general_dir=general_dir)
    except Exception as e:
        print(f"  [ERROR] Q model validation tables: {e}")
    ...
    try:
        ...
        plot_turning_arrow_diagram(
            movement_to_ids,
            out_path=arrow_path,
            title="Q Intersection - Detected Vehicle Movements",
        )
    except Exception as e:
