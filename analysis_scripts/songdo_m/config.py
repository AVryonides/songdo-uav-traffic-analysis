...
def parse_args():
    ...
    ap.add_argument("--traj", required=True, help="Trajectory CSV, e.g. 2022-10-04_M_PM5.csv")
    ap.add_argument("--seg", required=False, help="Segmentation CSV, e.g. M.csv. If omitted, inferred from traj filename.")
    ap.add_argument("--seg_dir", default=".", help="Directory where segmentation CSVs live (default: current dir)")
    ap.add_argument("--out", default="outputs_M", help="Output parent directory")
...
def patch_od_pairs():
    """No-op: M intersection uses arm-number OD pairs; no refinement needed."""
    pass
