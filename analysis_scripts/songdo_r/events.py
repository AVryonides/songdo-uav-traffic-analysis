...
from .r_rules import ARMS, R_ARM_APPROACH
...
def _base_section(section_value) -> str:
    """Strip lane suffix from values like '1_4_5' -> '1_4'."""
...
def _compute_vehicle_arrival_departure_times(
    ...
    Departure = time the vehicle crosses the stop line (approach → intersection).
    """
    # Movement -> approach (origin) sections for R intersection.
    # Use R_ARM_APPROACH, not departure sections, for stop-line crossing.
    arm_origin_sections = {
        f"{o}->{d}": set(R_ARM_APPROACH[o])
        for o in ARMS
        for d in ARMS
        if o != d
    }
        ...
        if mov and mov in arm_origin_sections:
            ...
            if len(origin_indices) > 0:
                ...
                if last_origin_idx < len(times) - 1:
                    t_dep = float(0.5 * (times[last_origin_idx] + times[last_origin_idx + 1]))
...
def compute_movement_arrival_departure_times(
    ...
    arrivals_by_vid, departures_by_vid = _compute_vehicle_arrival_departure_times(
        df, allowed_vehicle_ids=all_ids,
        vehicle_id_col=vehicle_id_col, time_col=time_col,
        speed_col=speed_col, section_col=section_col,
        x_col=x_col, y_col=y_col, g_df=g_df,
    ...
    for movement, vids in movement_to_ids.items():
        ...
        out[movement] = {"arrivals": np.sort(arr), "departures": np.sort(dep)}
...
def compute_lane_arrival_departure_times(
    ...
    out = {}
    for movement, vids in movement_to_ids.items():
        for vid in [int(v) for v in vids]:
            lane_val = per_vid_lane.get(vid)
            if pd.isna(lane_val):
                continue
            key = (movement, int(lane_val))
            if key not in out:
