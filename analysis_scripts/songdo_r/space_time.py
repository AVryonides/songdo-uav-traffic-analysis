...
from .r_rules import infer_r_lane_from_sequence as infer_q_lane_from_sequence, get_lane_path_description, movement_to_name
...
class SpaceTimeDiagram:
    ...
    # ── Movement / arm constants (R intersection — 4 arms, 12 movements) ──
    # Approach sections: vehicles COMING FROM each arm (used for origin detection
    # and stop-line reference normalisation).  Matches R_ARM_APPROACH in r_rules.py.
    _ARM1_IN = {"1_2", "1_3", "1_4"}        # North approach
    _ARM2_IN = {"2_3", "2_4"}               # West approach
    _ARM3_IN = {"3_3"}                      # South approach
    _ARM4_IN = {"4_2", "4_3", "4_4", "4_5"} # East approach

    # Departure sections: vehicles GOING TO each arm.  Matches R_ARM_DEPART in r_rules.py.
    _TO_ARM1 = {"1_5", "1_6"}              # departure TO North
    _TO_ARM2 = {"2_1", "2_2"}              # departure TO West
    _TO_ARM3 = {"3_1", "3_2"}              # departure TO South
    _TO_ARM4 = {"4_6"}                      # departure TO East
    ...
    _COMMON_GREEN_GROUPS = {
        "PHASE_ARM1": ["1->2", "1->3", "1->4"],
        "PHASE_ARM2": ["2->1", "2->3", "2->4"],
        "PHASE_ARM3": ["3->1", "3->2", "3->4"],
        "PHASE_ARM4": ["4->1", "4->2", "4->3"],
    }

    # No ground-truth phase data available for R; use empty list (inferred only).
    _OBSERVED_PHASES = []

    # Zoom windows not pre-calibrated for R — empty dict (no zoomed views generated).
    ...
    _MOV_TO_ARM_LABEL = {
        "1->2": "North Arm", "1->3": "North Arm", "1->4": "North Arm",
        "2->1": "West Arm",  "2->3": "West Arm",  "2->4": "West Arm",
        "3->1": "South Arm", "3->2": "South Arm", "3->4": "South Arm",
        "4->1": "East Arm",  "4->2": "East Arm",  "4->3": "East Arm",
    }

    # Movements observed as permissive in the UAV footage:
    # WS=2->3, NW=1->2, EN=4->1, SE=3->4.
    _PERMISSIVE_MOVS = frozenset({"2->3", "1->2", "4->1", "3->4"})
    ...
    _DIR_FULL_NAMES = {"1": "North", "2": "West", "3": "South", "4": "East"}
    ...
    def _movement_from_sections(self, origin, dest):
        # R intersection: 4 arms → 12 movements
        # Section values may carry a lane suffix (e.g. '4_3_8'); strip to base section.
        def _base(v):
            if v is None:
                return v
            parts = str(v).split('_')
            return '_'.join(parts[:2]) if len(parts) >= 3 else str(v)

        o_base = _base(origin)
        d_base = _base(dest)

        _arm_in = {
            "1": self._ARM1_IN, "2": self._ARM2_IN,
            "3": self._ARM3_IN, "4": self._ARM4_IN,
        }
        _arm_out = {
            "1": self._TO_ARM1, "2": self._TO_ARM2,
            "3": self._TO_ARM3, "4": self._TO_ARM4,
        }
        for o_arm, o_set in _arm_in.items():
            if o_base in o_set:
                for d_arm, d_set in _arm_out.items():
                    if o_arm != d_arm and d_base in d_set:
    ...
    def _origin_arm_set(self, mov):
        ...
        if "->" in str(mov):
            o_arm = str(mov).split("->")[0]
            return _map.get(o_arm, set())
        return set()

    def _origin_arm_label_from_section(self, section_value):
        s = str(section_value) if section_value is not None else ""
        # Strip lane suffix if present (e.g. '4_3_8' → '4_3')
        parts = s.split('_')
        base = '_'.join(parts[:2]) if len(parts) >= 3 else s
        if base in self._ARM1_IN:
            return "ARM1_SIGNAL"
        if base in self._ARM2_IN:
            return "ARM2_SIGNAL"
        if base in self._ARM3_IN:
            return "ARM3_SIGNAL"
        if base in self._ARM4_IN:
            return "ARM4_SIGNAL"
        return None

    @staticmethod
    def _dir_to_mov(direction_str):
        """Extract movement code from direction like 'North to West L_A' -> '1->2'."""
        _NAME_TO_ARM = {"North": "1", "West": "2", "South": "3", "East": "4"}
        parts = direction_str.replace("L_", "").split()
        if len(parts) >= 3 and parts[1] == "to":
            o = _NAME_TO_ARM.get(parts[0], parts[0])
            d = _NAME_TO_ARM.get(parts[2], parts[2])
            return o + "->" + d
        return direction_str

    _DISPLAY_LANE_OVERRIDES = {
        ("1->2", "lane_3"): "lane_A",  # North to West: raw origin lane 3 is display lane A
        ("1->2", "lane_A"): "lane_B",  # North to West: previous display A becomes B
        ("2->3", "lane_B"): "lane_A",  # West to South: collapse display B to A
    }
    ...
    def _display_lane_label(cls, mov, lane_label) -> str:
        raw = str(lane_label)
        return cls._DISPLAY_LANE_OVERRIDES.get((str(mov), raw), f"lane_{cls._lane_suffix_alpha(raw)}")

    @classmethod
    def _display_direction(cls, mov, lane_label) -> str:
        orig_arm, dest_arm = cls._MOV_ARM_MAP.get(mov, (str(mov)[:1], str(mov)[1:]))
        orig_name = cls._DIR_FULL_NAMES.get(orig_arm, orig_arm)
        dest_name = cls._DIR_FULL_NAMES.get(dest_arm, dest_arm)
        display_lane = cls._display_lane_label(mov, lane_label).replace("lane_", "")
        return f"{orig_name} to {dest_name} L_{display_lane}"

    @classmethod
    def _display_lane_maps(cls, mov, lane_labels):
        def _sort_key(lane):
            raw = str(lane)
            overridden = cls._display_lane_label(mov, raw).replace("lane_", "")
            if overridden.isalpha() and len(overridden) == 1:
                return (0, ord(overridden) - ord("A"), raw)
            suffix = raw.replace("lane_", "")
            if suffix.isdigit():
                return (1, int(suffix), raw)
            return (2, raw)

        ordered = sorted([str(l) for l in lane_labels if str(l) != "unknown"], key=_sort_key)
        ...
        for raw, suffix in zip(ordered, suffixes):
            direction_map[raw] = f"{orig_name} to {dest_name} L_{suffix}"
    ...
    def _filter_trajectories(self):
        ...
        for vid, group in df.groupby(vehicle_id_col):
            ...
            if mov == "UNK" and origin_arm_label is None:
                ...
                if pipeline_mov and pipeline_mov not in ("UNK", "UNASSIGNED"):
                    ...
                    if origin_set:
                        ...
                        # derive ARM_SIGNAL label from movement origin arm number
                        o_arm = str(mov).split("->")[0] if "->" in str(mov) else ""
                        origin_arm_label = f"ARM{o_arm}_SIGNAL" if o_arm else None
            ...
            # Section column may use 'SECTION_LANE' format (e.g. '4_3_8');
            # strip lane suffix for comparison with origin_set ('4_3' format).
            stop_idx = -1
            if section_cols:
                sc = section_cols[0]
                def _base_sec(v):
                    parts = str(v).split('_')
                    return '_'.join(parts[:2]) if len(parts) >= 3 else str(v)
                sec_stripped = g[sc].apply(_base_sec)
                mask = sec_stripped.isin(origin_set).values
    ...
    def _attempt_recovery(self):
        ...
        for row in self.reject_rows:
            ...
            from .r_rules import infer_r_movement_from_lane_sequence as infer_q_movement_from_lane_sequence
            vid_lane = "unknown"
            if self.vehicle_state_sequence is not None:
                seq = self.vehicle_state_sequence.get(int(vid), [])
                if mov == "UNK" and seq:
    ...
    def _plot_per_movement_per_lane(self):
        ...
        for mov in self._MOVEMENTS:
            ...
            for lane_label in lanes_in_mov:
                ...
                mov_name = movement_to_name(mov)  # "4->1" → "EN"
                display_lane_label = display_lane_map.get(str(lane_label), self._display_lane_label(mov, lane_label))
                lane_dir = os.path.join(run_dir, f"space_time_{mov_name}_{display_lane_label}")
                os.makedirs(lane_dir, exist_ok=True)
                lane_out = os.path.join(lane_dir, f"space_time_{mov_name}_{display_lane_label}{ext_local}")
                mov_zooms = self._ZOOM_WINDOWS_BY_MOVEMENT.get(mov, None)
                path_desc = get_lane_path_description(mov, lane_label)
                if path_desc:
                    lane_title = f"{self.title}: {mov_name}_{display_lane_label} ({path_desc}) {n_veh} veh"
                else:
                    lane_title = f"{self.title}: {mov_name}_{display_lane_label} {n_veh} veh"
            ...
            if lane_summaries:
                print(f"  {mov_name} ({mov}): {', '.join(lane_summaries)}")
    ...
    def _render_signal_timing_chart(self, df_phases, t_start, t_end, out_path, title,
        ...
        arm_order = ["North Arm", "West Arm", "South Arm", "East Arm"]
        y_labels = []
        seen = set()
