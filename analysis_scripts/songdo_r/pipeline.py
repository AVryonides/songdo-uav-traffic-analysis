...
def _load_green_intervals(general_dir: str) -> dict:
    ...
    def _dir_to_mov(d: str):
        ...
        if len(parts) >= 3 and parts[1] == "to":
            name_to_arm = {"North": "1", "West": "2", "South": "3", "East": "4"}
            origin = name_to_arm.get(parts[0], parts[0])
            dest = name_to_arm.get(parts[2], parts[2])
            if origin in {"1", "2", "3", "4"} and dest in {"1", "2", "3", "4"}:
...
def main():
    ...
    base_order = ["1->2","1->3","1->4","2->1","2->3","2->4","3->1","3->2","3->4","4->1","4->2","4->3"]
    ...
    try:
        ...
        plot_turning_arrow_diagram(
            movement_to_ids,
            out_path=arrow_path,
            title="R Intersection - Detected Vehicle Movements",
        )
    except Exception as e:
