...
def plot_cumulative_by_movement(
    ...
    with plt.rc_context(_THESIS_FONT):
        _MOVEMENT_LABELS = {
            "1->2": "North to West",  "1->3": "North to South", "1->4": "North to East",
            "2->1": "West to North",  "2->3": "West to South",  "2->4": "West to East",
            "3->1": "South to North", "3->2": "South to West",  "3->4": "South to East",
            "4->1": "East to North",  "4->2": "East to West",   "4->3": "East to South",
        }
...
def plot_cumulative_by_lane(
    ...
    with plt.rc_context(_THESIS_FONT):
        ...
        # Descriptive direction labels for folder naming (R intersection, 1=N 2=W 3=S 4=E)
        _MOVEMENT_DESCRIPTIONS = {
            "1->2": "North_to_West",  "1->3": "North_to_South", "1->4": "North_to_East",
            "2->1": "West_to_North",  "2->3": "West_to_South",  "2->4": "West_to_East",
            "3->1": "South_to_North", "3->2": "South_to_West",  "3->4": "South_to_East",
            "4->1": "East_to_North",  "4->2": "East_to_West",   "4->3": "East_to_South",
        }
...
def plot_turning_arrow_diagram(direction_to_ids: dict, out_path: str, title: str = "Movements"):
    """Movement count table for M intersection (12 arm-pair movements arranged in a 4×3 grid)."""
    # M movement order: origins 1-4 × destinations excluding self
    _ARM_LABELS = {"1": "Arm 1\n(NW)", "2": "Arm 2\n(S)", "3": "Arm 3\n(SE/E)", "4": "Arm 4\n(NE)"}
    arms = ("1", "2", "3", "4")
    move_order = [f"{o}->{d}" for o in arms for d in arms if o != d]

    counts = {}
    for k, vids in (direction_to_ids or {}).items():
        k_str = str(k).strip()
        # Accept both "1->2" and "1→2" notation
        k_norm = k_str.replace("→", "->")
        counts[k_norm] = len(vids) if hasattr(vids, "__len__") else 0

    rows, cols = 4, 3  # 4 origin arms × 3 destination movements
    fig_w, fig_h = cols * 2.6, rows * 2.0
    fig, axes = plt.subplots(rows, cols, figsize=(fig_w, fig_h))

    # Map origin arm to row, destination index within origin to column
    arm_dest_map = {o: [d for d in arms if d != o] for o in arms}

    for r_idx, origin in enumerate(arms):
        dests = arm_dest_map[origin]
        for c_idx, dest in enumerate(dests):
            ax = axes[r_idx][c_idx]
            mov = f"{origin}->{dest}"
            cnt = counts.get(mov, 0)
            ax.set_xlim(0, 1)
            ax.set_ylim(0, 1)
            ax.set_xticks([])
            ax.set_yticks([])
            ...
            # Movement arrow (horizontal for visual simplicity)
            ax.annotate("", xy=(0.75, 0.55), xytext=(0.25, 0.55),
                        arrowprops=dict(arrowstyle="-|>", color="black",
                                        mutation_scale=14, lw=1.8))
            # Origin / destination labels
            ax.text(0.13, 0.55, _ARM_LABELS.get(origin, origin),
                    ha="center", va="center", fontsize=7, color="#333")
            ax.text(0.87, 0.55, _ARM_LABELS.get(dest, dest),
                    ha="center", va="center", fontsize=7, color="#333")
            # Movement name
            ax.text(0.5, 0.82, mov, ha="center", va="center",
                    fontsize=11, fontweight="bold", color="black")
            # Vehicle count badge
            bg = "#C8E6C9" if cnt > 0 else "#FFCDD2"
            ec = "#2E7D32" if cnt > 0 else "#B71C1C"
            ax.text(0.5, 0.25, f"{cnt} vehicles",
...
def plot_intersection_layout(out_path: str, title: str = "R Intersection — Layout & Movement Directions"):
    ...
    C = {
        'SN': '#E53935',  # red
        'SW': '#FB8C00',  # orange
        'SE': '#D4A017',  # dark golden-yellow (adjusted for readability)
    ...
    with plt.rc_context(_THESIS_FONT):
        ...
        xL = -7 * lw   # West edge (North arm has 7 incoming lanes)
        xR = +5 * lw   # East edge (South arm has 5 incoming lanes)
        yT = +7 * lw   # North edge (East arm has 7 incoming lanes)
        yB = -6 * lw   # South edge (West arm has 6 incoming lanes)
        ...
        for i in range(1, 4): ax.plot([i * 1.25 * lw] * 2, [yT + gap, yT + arm - 0.25], **dkw)
        # South
        for i in range(1, 4): ax.plot([-i * 1.75 * lw] * 2, [yB - gap, yB - arm + 0.25], **dkw)
        for i in range(1, 5): ax.plot([i * lw] * 2, [yB - gap, yB - arm + 0.25], **dkw)
        # West
        for i in range(1, 5): ax.plot([xWL + 0.25, xL - gap], [i * 1.4 * lw] * 2, **dkw)
        for i in range(1, 6): ax.plot([xWL + 0.25, xL - gap], [-i * lw] * 2, **dkw)
        # East
        for i in range(1, 7): ax.plot([xR + gap, xER - 0.25], [i * lw] * 2, **dkw)
        for i in range(1, 5): ax.plot([xR + gap, xER - 0.25], [-i * 1.2 * lw] * 2, **dkw)
        ...
        ax.plot([+3.75 * lw] * 2, [yT + gap, yT + arm - 0.25], **gkw)
        # South
        ax.plot([-5.25 * lw] * 2, [yB - gap, yB - arm + 0.25], **gkw)
        ax.plot([+1 * lw] * 2, [yB - gap, yB - arm + 0.25], **gkw)
        ax.plot([+4 * lw] * 2, [yB - gap, yB - arm + 0.25], **gkw)
        # West
        ax.plot([xWL + 0.25, xL - gap], [+4.2 * lw] * 2, **gkw) 
        ax.plot([xWL + 0.25, xL - gap], [-1 * lw] * 2, **gkw)
        ax.plot([xWL + 0.25, xL - gap], [-5 * lw] * 2, **gkw)
        # East
        ax.plot([xR + gap, xER - 0.25], [+6 * lw] * 2, **gkw)
        ax.plot([xR + gap, xER - 0.25], [+3 * lw] * 2, **gkw)
        ax.plot([xR + gap, xER - 0.25], [-3.6 * lw] * 2, **gkw)

        # ── stop lines ───────────────────────────────────────────
        skw = dict(color="#FFC107", lw=4.0, zorder=4, solid_capstyle="butt")
        ax.plot([xL, -gap],  [yT, yT],  **skw)   # N incoming (West side)
        ax.plot([gap, xR],   [yB, yB],  **skw)   # S incoming (East side)
        ax.plot([xL, xL],    [-gap, yB], **skw)  # W incoming (South side)
        ax.plot([xR, xR],    [gap, yT],  **skw)  # E incoming (North side)
        ...
        _vband(0, 3.75*lw,      ytop_N, yT, C['SN'])   
        _vband(3.75*lw, 5*lw,   ytop_N, yT, C['EN'])   

        # South arm bands
        _vband(-7*lw, -5.25*lw, yB, ybot_S, C['WS'])    
        _vband(-5.25*lw, 0,     yB, ybot_S, C['NS'])   
        _vband(0, 1*lw,         yB, ybot_S, C['SW'])   
        _vband(1*lw, 4*lw,      yB, ybot_S, C['SN'])   
        _vband(4*lw, 5*lw,      yB, ybot_S, C['SE'])   

        # West arm bands
        _hband(xWL, xL, 4.2*lw, 7*lw,   C['NW'])   
        _hband(xWL, xL, 0, 4.2*lw,      C['EW'])   
        _hband(xWL, xL, 0, -1*lw,       C['WN'])   
        _hband(xWL, xL, -1*lw, -5*lw,   C['WE'])   
        _hband(xWL, xL, -5*lw, -6*lw,   C['WS'])   

        # East arm bands
        _hband(xR, xER, 6*lw, 7*lw,     C['EN'])   
        _hband(xR, xER, 3*lw, 6*lw,     C['EW'])   
        _hband(xR, xER, 0, 3*lw,        C['ES'])   
        _hband(xR, xER, 0, -3.6*lw,     C['WE'])   
        _hband(xR, xER, -3.6*lw, -6*lw, C['SE'])   
        ...
        # North arm
        draw_lane(-6.5 * lw, ay_N, "NW (1)", C['NW'], "left", 0)      # Swapped back to 'left'
        draw_lane(-4.5 * lw, ay_N, "NS (3)", C['NS'], "straight", 0)
        draw_lane(-1.5 * lw, ay_N, "NE (3)", C['NE'], "right", 0)     # Swapped back to 'right'
        draw_lane(+1.875 * lw, ay_N, "SN (3)", C['SN'], "straight", 180) 
        draw_lane(+4.375 * lw, ay_N, "EN (1)", C['EN'], "left", 180)  

        # South arm
        draw_lane(-6.125 * lw, ay_S, "WS (1)", C['WS'], "left", 0)    
        draw_lane(-2.625 * lw, ay_S, "NS (3)", C['NS'], "straight", 0)  
        draw_lane(+0.5 * lw, ay_S, "SW (1)", C['SW'], "right", 180)   
        draw_lane(+2.5 * lw, ay_S, "SN (3)", C['SN'], "straight", 180)
        draw_lane(+4.5 * lw, ay_S, "SE (1)", C['SE'], "left", 180)    

        # West arm
        draw_lane(ax_W, +5.6 * lw, "NW (2)", C['NW'], "left", -90)       
        draw_lane(ax_W, +2.1 * lw, "EW (3)", C['EW'], "straight", -90) 
        draw_lane(ax_W, -0.5 * lw, "WN (1)", C['WN'], "right", 90)    
        draw_lane(ax_W, -3.0 * lw, "WE (4)", C['WE'], "straight", 90)
        draw_lane(ax_W, -5.5 * lw, "WS (1)", C['WS'], "left", 90)     

        # East arm
        draw_lane(ax_E, +6.5 * lw, "EN (1)", C['EN'], "left", -90)    
        draw_lane(ax_E, +4.5 * lw, "EW (3)", C['EW'], "straight", -90)
        draw_lane(ax_E, +1.5 * lw, "ES (3)", C['ES'], "right", -90)   
        draw_lane(ax_E, -1.8 * lw, "NE+WE (3)", C['WE'], "straight", 90)
        draw_lane(ax_E, -4.8 * lw, "SE (2)", C['SE'], "left", 90)    
        ...
        ax.text(xR + 0.25, yB - arm / 2, "OUTGOING (5 lanes)", **side)

        side_h = dict(fontsize=8, ha="center", va="center", color="#777777", zorder=5)
        ax.text((xWL + xL) / 2, yT + 0.20, "INCOMING (5 lanes)", **side_h)
        ax.text((xWL + xL) / 2, yB - 0.20, "OUTGOING (6 lanes)", **side_h)
        ax.text((xR + xER) / 2, yT + 0.20, "OUTGOING (7 lanes)", **side_h)
        ax.text((xR + xER) / 2, yB - 0.20, "INCOMING (5 lanes)", **side_h)
